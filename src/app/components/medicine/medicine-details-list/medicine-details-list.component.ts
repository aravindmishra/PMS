...
export class MedicineDetailsListComponent implements OnInit {
  ...
  public getMedicineList()
  {
    this.commonService.post(this.URLs.api.medicine_list).subscribe((response:any)=>{
      if(!response.error)
      {
        ...
        this.medicineService.allocatedRackList = this.medicineList.map((element:any)=>element.rack_no);
        console.log(this.medicineService.medicineNameList,this.medicineService.allocatedRackList)
      }
    })
  }
  ...
  public searchFilter(searchValue:string)
  {
    if(searchValue)
    {
      this.filterMedicineList = this.medicineList.filter((element:any)=>{
        element.medicine_name.toLowerCase().includes(searchValue);
      });
      console.log(this.filterMedicineList);
    }
    else
    {
      this.filterMedicineList = this.medicineList;
